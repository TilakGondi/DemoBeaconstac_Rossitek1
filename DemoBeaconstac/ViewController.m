...
//    BOOL success = [Beaconstac setupOrganizationId:<org_ID_value> userToken:<developer_token_value> beaconUUID:<beaconUUID_value> beaconIdentifier:<com.<company_name_value>.<app_name_value>>];
    ...
    BOOL success = [Beaconstac setupOrganizationId:85
                                         userToken:@"ec261c421ecff94fb31b38d2a895835e0c8dbbba"
                                        beaconUUID:@"B9407F30-F5F8-466E-AFF9-25556B57FE6D"
                                  beaconIdentifier:@"com.rossitek.BeconstacDemo"];
...
{
    [beaconstac updateFact:@"female" forKey:@"Gender"];
    [beaconstac updateFact:@"05:13 PM" forKey:@"TeaTime"];
//    [beaconstac updateFact:@"" forKey:@""];
}
...
- (void)ruleTriggeredWithRuleName:(NSString *)ruleName actionArray:(NSArray *)actionArray
{
    ...
    for (NSDictionary *actionDict in actionArray) {
        ...
        if ([[[actionDict objectForKey:@"meta"]objectForKey:@"action_type"]  isEqual: @"popup"]) {
            ...
            [[[UIAlertView alloc] initWithTitle:ruleName message:[NSString stringWithFormat:@"%@",message] delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event
{
    ...
    OfferTableViewController *offerVC=[self.storyboard instantiateViewControllerWithIdentifier:@"Offer"];
    offerVC.nearBeacon=[beaconstac.factsDictionary valueForKey:@"nearbeaconName"];
